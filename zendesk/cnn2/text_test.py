...
tf.app.flags.DEFINE_string("test_file", "sentiment/clean_en_l10.csv", 
                           "Data source for the test data.")
tf.app.flags.DEFINE_string("model_path", "models/en", 
                           "Folder to save the model")
tf.app.flags.DEFINE_string("pretrained_embedding_file", 
                           "pretrained_embedding/glove.6B.100d.txt", 
                           "Data source for the test data.")
...
tf.app.flags.DEFINE_string("filter_sizes", "1,2,3", 
                           "Comma-separated filter sizes (default: '1,2,3')")
tf.app.flags.DEFINE_integer("num_filters", 25, 
                            "Number of filters per filter size (default: 25)")
tf.app.flags.DEFINE_float("dropout_keep_prob", 0.5, 
                          "Dropout keep probability (default: 0.5)")
tf.app.flags.DEFINE_float("l2_reg_lambda", 0.1, 
                          "L2 regularization lambda (default: 0.0)")
tf.app.flags.DEFINE_integer("oov_freq_threshold", 5, 
                          "OOV word frequency threshold (default: 5)")
...
tf.app.flags.DEFINE_integer("max_doc_length", 128, 
                            "Length (default: 128)")
...
def _transform(x,  table, max_doc_len):
    def _pad_unk(u):
        ...
        return tf.pad(u, [[0,0],[0,max_doc_len-tf.shape(u)[1]]], 
                      'CONSTANT',constant_values='<UNK>')[0]
    ...
    def body(i, v, r):
        ...
        u = tf.cond(tf.shape(u)[0] >= max_doc_len, 
                    lambda: tf.slice(u, [0], [max_doc_len]), 
                    lambda: _pad_unk(u))
...
def add_training_words(vocab, train_texts, sess):
    ...
    for batch in batches:
        ...
        for t in ts:
            t = t.decode()
            if (len(t) <= 32) and (t not in existing_words) :
                new_words.append(t)
                #new_words.add(t)
    wordcount = dict(Counter(new_words).most_common())
    new_words = dict([x for x in wordcount.items() if x[1] >= FLAGS.oov_freq_threshold])
    print(new_words)
    new_words = [x[0] for x in new_words.items()]
    vocab.extend(list(new_words))

def main(_):
           
    x_train,y_train,x_test,y_test,labels = data_helpers.load_data_and_labels(
                                           FLAGS.train_file,                                                                                        FLAGS.test_file)
    ...
    print("pretrained words: " +str(len(embedding))
          +" uncovered training words: " +str(len(vocab)-len(embedding)))

    if len(embedding) > 0:
        embedding = np.concatenate((embedding, 
                           np.zeros([len(vocab)-len(embedding),len(embedding[0])],                                                  dtype=np.float32)), axis=0)
    else:
        embedding = np.zeros([len(vocab), 100], dtype=np.float32)
    ...
    table = _build_tensor_vocabulary_processor(
           vocab_processor.vocabulary_._mapping, sess
                                              )


    serialized_tf_example = tf.placeholder(tf.string, name='tf_example')
    feature_configs = {"x": tf.FixedLenFeature([], dtype=tf.string)}
  ...
  # Final (unnormalized) scores and predictions
    with tf.name_scope("output"):
        ...
        dropped_scores = tf.nn.xw_plus_b(h_drop, 
                                         W, 
                                         b, 
                                         name="dropped_scores")
        scores = tf.nn.xw_plus_b(h_pool_flat, 
                                 W, 
                                 b, 
                                 name="scores")
        y = tf.argmax(scores, 
                      1, 
                      name="predictions")
        sorted_scores, sorted_indices = tf.nn.top_k(scores, 
                                                    num_classes, 
                                                    name='sorted_scores')
        sorted_labels = tf.gather(labels,
                                  sorted_indices, 
                                  name='sorted_labels')

  # Calculate mean cross-entropy loss
    with tf.name_scope("loss"):
        losses = tf.nn.softmax_cross_entropy_with_logits(logits=dropped_scores, 
                                                         labels=input_y)
  ...
  # Accuracy
    ...
    require_improvement = 10000 # Stop optimization if no improvement found in this many steps.
  ...
  # Checkpoint directory. Tensorflow assumes this directory already exists so need to create it
    ...
    for batch in batches:
        ...
        _, step, lo, ac = sess.run([train_op, 
                                    global_step, 
                                    loss, 
                                    accuracy],
        ...
        if current_step % FLAGS.evaluate_every == 0:
            ...
            test_batches = data_helpers.batch_iter(list(zip(x_test, y_test)), 
                                                   FLAGS.batch_size, 
                                                   1)
            acc = 0
            los = 0
            for test_batch in test_batches:
                ...
                _, lo, ac = sess.run([global_step, loss, accuracy],
            ...
            if acc > best_validation_accuracy:
                ...
                print("Updated best model checkpoint to {}\n".format(path)) 

            print("Best accuracy on test : {:g}".format(best_validation_accuracy))
            print("Best step : {:g}".format(best_step))           
            print("Test: loss {:g}, acc {:g}".format(los, acc))
            print("Test: step {:g}\n".format(step))
            
            if step - best_step > require_improvement:
                print("No improvements for {} steps. Exporting model".format(require_improvement))
                break
